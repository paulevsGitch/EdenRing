...
public class BalloonMushroomBlockItem extends BlockItem {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
		...
		BlockHitResult hit = level.isBlockInLine(new ClipBlockStateContext(
			...
			player.getEyePosition().add(vec.scale(5)),
			state -> state.isAir()
		));
		
		if (hit != null && hit instanceof BlockHitResult) {
			level.gameEvent(player, GameEvent.BLOCK_PLACE, hit.getBlockPos());
			level.setBlock(hit.getBlockPos(), getBlock().defaultBlockState(), 0);
		}
