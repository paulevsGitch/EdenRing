...
public class MultiThreadGenerator {
	...
	public static void init(long seed, Sampler sampler, BiomeSource biomeSource) {
		MultiThreadGenerator.biomeSource = biomeSource;
	...
	public static TerrainGenerator getTerrainGenerator() {
		return TERRAIN_POOL.computeIfAbsent(Thread.currentThread().getId(), i -> new TerrainGenerator(seed, sampler, biomeSource));
	}
	
	public static TerrainGenerator getBiomeGenerator() {
		return BIOME_POOL.computeIfAbsent(Thread.currentThread().getId(), i -> new TerrainGenerator(seed, sampler, biomeSource));
	}
